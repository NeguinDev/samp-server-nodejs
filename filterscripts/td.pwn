...
new PlayerText:TDGaragem[MAX_PLAYERS][27];
...
public OnPlayerCommandText(playerid, cmdtext[]) {
    ...
    if (!strcmp("/td", cmdtext)) {
		SendClientMessage(playerid, -1, "AAAA");

        for (new i = 0; i < sizeof TDGaragem; i++) PlayerTextDrawShow(playerid, TDGaragem[playerid][i]);
    }
